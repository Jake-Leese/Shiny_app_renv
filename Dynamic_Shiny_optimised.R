...
# Load "Full Data" and "HH6" ArchR objects once and keep in memory
FullData_ArchR <- loadArchRProject(path = archr_paths[["Full Data"]], showLogo = FALSE)
HH6_ArchR <- loadArchRProject(path = archr_paths[["HH6"]], showLogo = FALSE)
...
# Have checked that this final list of 384 TFs is the same across all stages, using FullData as already loaded at this point
motifs <- unique(gsub("^(z:|deviations:)", "", getFeatures(FullData_ArchR, useMatrix = "MotifMatrix")))
genes <- getFeatures(FullData_ArchR, useMatrix = "GeneScoreMatrix")
matches_gene_score <- sapply(motifs, function(x) any(grepl(x, genes)))
TF_options_1 <- sort(motifs[matches_gene_score])

int_genes <- getFeatures(FullData_ArchR, useMatrix = "GeneIntegrationMatrix")
matches <- sapply(TF_options_1, function(x) any(grepl(x, int_genes)))
TF_options <- sort(TF_options_1[matches])
...
server <- function(input, output, session) {
  ...
  # Reactive value to store the ArchR object
  archr_object <- reactiveVal(NULL)
  
  # Initial load: Full Data
  observe({
    isolate({
      archr_object(FullData_ArchR)
      updateRadioButtons(session, "UMAP_subset_ArchR", selected = "Full Data")
      updateRadioButtons(session, "dimplot_ArchR_groupby", selected = "Stage")
    })
  })
  
  # Update ArchR object when dataset selection changes
  ...
  # UMAP plot
  observe({
    req(archr_object(), input$dimplot_ArchR_groupby)
    
    output$dimplot_ArchR <- renderPlot({
      ArchR_dimplot(archr_object(), name = input$dimplot_ArchR_groupby) +
        theme_minimal()
    }, height = function() {
      session$clientData$output_dimplot_ArchR_width * 0.8
    })
  })
  
  # Feature plot
  observe({
    req(archr_object(), input$featureplot_ArchR_TF, input$featureplot_ArchR_datatype)
    
    output$featureplot_ArchR <- renderPlot({
      ArchR_featureplot(
        archr_object(),
        TF = input$featureplot_ArchR_TF,
        datatype = input$featureplot_ArchR_datatype
      ) + theme_minimal()
    }, height = function() {
      session$clientData$output_dimplot_ArchR_width * 0.8
    })
  ...
  # Genome Browser dataset selection
  ...
  observeEvent(input$run_gBrowser, {
    req(archr_object(), input$gbrowser_ArchR_groupby, input$gbrowser_ArchR_region)
...
tab_ArchR_umap <- tabItem(tabName = "ArchR_UMAP",
                          ...
                          fluidRow(
                            ...
                            column(6,
                                   fluidRow(
                                     column(12,
                                            radioButtons("UMAP_subset_ArchR", "Data subset to plot", data_subsets, inline = TRUE, selected = "Full Data", width = '800')
                                     )
                                   ),
                                   fluidRow(
                                     column(12,
                                            radioButtons("dimplot_ArchR_groupby", "UMAP coloured by", ArchR_groupby_options, inline = TRUE, selected = "Stage", width = '800')
                                     )
                                   ),
                                   fluidRow(
                                     column(12,
...
tab_ArchR_gbrowser <- tabItem(tabName = "ArchR_Genome_Browser",
                              fluidRow(
                                column(12,
                                       radioButtons("gbrowser_subset_ArchR", "Data subset to plot", data_subsets, inline = TRUE, selected = 'Full Data', width = '800')
                                )
                              ),
                              fluidRow(
                                column(12,
                                       radioButtons("gbrowser_ArchR_groupby", "How to group cells", ArchR_groupby_options, inline = TRUE, selected = 'Stage', width = '800')
                                )
                              ),
                              fluidRow(
